...
contract NGO_Funding {
    ...
    function register(string calldata _uri) external returns(uint _idx){
          ++idx;
    ...
    function createRequest(uint  _idx , string memory _uri, address _recipient, uint _amount) external onlyNgo(_idx) {
    ...
    function voteOnRequest(uint _idx, bool voteYes) external onlyDonor(_idx) {
    ...
    function finalizeRequest(uint _requestIdx, uint _ngoIdx) external {
